...
const OPERATOR_GROUPS = {
  '比较': ['equals','not_equals','equals_ignore_case','greater_than','less_than','greater_equal','less_equal','between','not_between'],
  '字符串': ['contains','not_contains','contains_ignore_case','starts_with','ends_with','matches_regex','not_matches_regex'],
  '存在性': ['exists','not_exists','is_empty','is_not_empty','is_true','is_false'],
  '变化': ['changed','not_changed','increased','decreased'],
...
const OPERATOR_LABELS = {
  equals:'等于', not_equals:'不等于', equals_ignore_case:'等于(忽略大小写)',
  greater_than:'大于', less_than:'小于',
  greater_equal:'大于等于', less_equal:'小于等于',
  between:'介于', not_between:'不介于',
  contains:'包含', not_contains:'不包含', contains_ignore_case:'包含(忽略大小写)',
  ...
  is_true:'为真', is_false:'为假',
  changed:'发生变化', not_changed:'未变化',
  increased:'增加', decreased:'减少',
...
const STATE_GROUP_LABELS = {
  ...
  'usb': '🔌 USB', 'service': '🛠️ 服务', 'bluetooth': '🔵 蓝牙',
  'power_plan': '⚡ 电源计划',
  'keyboard': '⌨️ 键盘', 'cursor': '🖱️ 光标', 'window_mode': '🪟 窗口模式',
  'input_language': '🌐 输入法', 'theme': '🎨 主题', 'pending_reboot': '🔄 待重启',
  'proxy': '🌐 代理', 'printer': '🖨️ 打印机', 'wallpaper': '🖼️ 壁纸',
  'recycle_bin': '🗑️ 回收站', 'gpu': '🎮 显卡', 'public_ip': '🌍 公网 IP',
  'startup': '🚀 启动项', 'scheduled_task': '📅 计划任务', 'hosts': '📝 Hosts',
  'firewall': '🛡️ 防火墙', 'battery_saver': '🔋 节电',
};
...
const STATE_KEY_LABELS = {
  ...
  // 新增插件状态键
  'usb.count': 'USB 设备数量',
  'usb.names': 'USB 设备名称列表',
  'bluetooth.count': '蓝牙设备数量',
  'bluetooth.connected_count': '蓝牙已连接数量',
  'bluetooth.has_connected': '是否有已连接蓝牙设备',
  'power_plan.active': '当前电源计划',
  'power_plan.is_balanced': '是否平衡模式',
  'power_plan.is_power_saver': '是否节能模式',
  'power_plan.is_high_performance': '是否高性能模式',
  'keyboard.caps_lock': 'CapsLock 状态',
  'keyboard.num_lock': 'NumLock 状态',
  'keyboard.scroll_lock': 'ScrollLock 状态',
  'cursor.x': '光标 X 坐标',
  'cursor.y': '光标 Y 坐标',
  'window_mode.is_fullscreen': '前台窗口是否全屏',
  'input_language.code': '键盘布局代码',
  'input_language.name': '键盘布局名称',
  'input_language.is_chinese': '是否中文输入法',
  'theme.mode': '主题 dark/light',
  'theme.is_dark': '是否深色模式',
  'pending_reboot.pending': '是否等待重启',
  'pending_reboot.reason': '待重启原因',
  'proxy.enabled': '代理是否开启',
  'proxy.server': '代理服务器地址',
  'printer.default': '默认打印机',
  'printer.has_default': '是否有默认打印机',
  'wallpaper.path': '壁纸路径',
  'recycle_bin.count': '回收站文件数',
  'recycle_bin.size_mb': '回收站占用 MB',
  'recycle_bin.is_empty': '回收站是否为空',
  'gpu.name': '显卡名称',
  'public_ip.ip': '公网 IP',
  'startup.count': '启动项数量',
  'startup.names': '启动项列表',
  'scheduled_task.name': '计划任务名',
  'scheduled_task.status': '计划任务状态',
  'scheduled_task.found': '计划任务是否存在',
  'scheduled_task.is_running': '计划任务是否运行中',
  'hosts.mtime': 'Hosts 修改时间',
  'hosts.lines': 'Hosts 行数',
  'hosts.changed': 'Hosts 是否刚变更',
  'firewall.domain': '域配置文件防火墙',
  'firewall.private': '专用配置文件防火墙',
  'firewall.public': '公用配置文件防火墙',
  'firewall.all_enabled': '防火墙是否全开',
  'battery_saver.enabled': '是否节电模式',
};
...
const OUTPUT_PARAM_HELP = {
  ...
  'mute':          { desc:'控制系统静音/取消静音', params: { mute:{type:'string',required:false,hint:'true=静音 / false=取消静音',desc:'设置静音状态；省略则切换'}, toggle:{type:'string',required:false,hint:'true=切换静音',desc:'true 时切换当前静音状态'} } },
  'restart_explorer': { desc:'重启 Windows 资源管理器 (explorer.exe)', params: {} },
  'volume_fade':   { desc:'将音量平滑渐变到目标值', params: { level:{type:'integer',required:true,hint:'目标音量 0-100',desc:'渐变目标音量'}, duration_ms:{type:'integer',required:false,hint:'渐变时长(ms)',desc:'默认 1500ms'}, steps:{type:'integer',required:false,hint:'步数',desc:'默认 20'} } },
  'control_panel': { desc:'打开控制面板页或 Windows 设置页', params: { page:{type:'string',required:true,hint:'控制面板项或 ms-settings: URI',desc:'如 Microsoft.Windows.Update 或 ms-settings:network-wifi'} } },
  'kill_process_name': { desc:'按进程名终止全部匹配进程', params: { name:{type:'string',required:true,hint:'进程名 (可含/不含 .exe)',desc:'将 taskkill /f /im 终止所有同名进程'} } },
  'media_control': { desc:'发送系统媒体按键（播放器/音量）', params: { action:{type:'string',required:true,hint:'play_pause/next/prev/stop/volume_up/volume_down',desc:'要发送的媒体键'} } },
  'flush_dns': { desc:'清空 DNS 解析缓存 (ipconfig /flushdns)', params: {} },
  'dark_mode': { desc:'切换系统深色/浅色主题', params: { mode:{type:'string',required:true,hint:'dark / light',desc:'目标主题模式'} } },
  'set_dns': { desc:'设置或重置网卡 DNS', params: { adapter:{type:'string',required:true,hint:'适配器名称',desc:'如「以太网」「WLAN」'}, server:{type:'string',required:false,hint:'DNS 服务器',desc:'如 8.8.8.8；留空=自动获取'} } },
  'empty_recycle_bin': { desc:'清空回收站', params: {} },
  'window_topmost': { desc:'将标题匹配的窗口设为置顶', params: { title:{type:'string',required:true,hint:'窗口标题子串',desc:'模糊匹配窗口标题'}, topmost:{type:'string',required:false,hint:'true/false',desc:'默认 true=置顶'} } },
  'process_priority': { desc:'设置进程优先级', params: { name:{type:'string',required:true,hint:'进程名',desc:'可不含 .exe'}, priority:{type:'string',required:false,hint:'idle/below_normal/normal/above_normal/high',desc:'默认 normal'} } },
  'notify_discord': { desc:'发送 Discord Webhook 消息', params: { webhook_url:{type:'string',required:true,hint:'Webhook URL',desc:'Discord 频道 Webhook'}, message:{type:'string',required:true,hint:'消息内容',desc:'要发送的文本'}, username:{type:'string',required:false,hint:'机器人用户名',desc:'可选显示名'} } },
  'notify_bark': { desc:'Bark iOS 推送', params: { device_key:{type:'string',required:true,hint:'设备 Key',desc:'Bark App 中的 device_key'}, server:{type:'string',required:false,hint:'服务器',desc:'默认 https://api.day.app'}, title:{type:'string',required:false,hint:'标题',desc:'通知标题'}, message:{type:'string',required:true,hint:'内容',desc:'推送正文'} } },
  'notify_wecom': { desc:'企业微信群机器人', params: { webhook_url:{type:'string',required:false,hint:'Webhook URL',desc:'完整机器人 URL'}, key:{type:'string',required:false,hint:'key',desc:'或只填 key'}, message:{type:'string',required:true,hint:'消息',desc:'文本消息'} } },
  'notify_dingtalk': { desc:'钉钉群机器人', params: { webhook_url:{type:'string',required:true,hint:'Webhook URL',desc:'含加签时可直接带 sign'}, message:{type:'string',required:true,hint:'消息',desc:'文本消息'} } },
  'notify_feishu': { desc:'飞书群机器人', params: { webhook_url:{type:'string',required:true,hint:'Webhook URL',desc:'自定义机器人地址'}, message:{type:'string',required:true,hint:'消息',desc:'文本消息'} } },
  'notify_serverchan': { desc:'Server酱 Turbo 推送到微信', params: { sendkey:{type:'string',required:true,hint:'SendKey',desc:'SCT 开头的 key'}, title:{type:'string',required:false,hint:'标题',desc:'默认 RuleCraft'}, message:{type:'string',required:true,hint:'内容',desc:'推送正文'} } },
  'notify_telegram': { desc:'Telegram Bot 消息', params: { bot_token:{type:'string',required:true,hint:'Bot Token',desc:'BotFather 的 token'}, chat_id:{type:'string',required:true,hint:'Chat ID',desc:'目标会话 ID'}, message:{type:'string',required:true,hint:'消息',desc:'文本内容'} } },
  'notify_slack': { desc:'Slack Incoming Webhook', params: { webhook_url:{type:'string',required:true,hint:'Webhook URL',desc:'Slack App Webhook'}, message:{type:'string',required:true,hint:'消息',desc:'文本内容'} } },
  'notify_homeassistant': { desc:'Home Assistant REST API', params: { base_url:{type:'string',required:true,hint:'HA 地址',desc:'如 http://homeassistant.local:8123'}, token:{type:'string',required:true,hint:'长期访问令牌',desc:'Bearer Token'}, message:{type:'string',required:true,hint:'消息',desc:'通知内容'}, title:{type:'string',required:false,hint:'标题',desc:'通知标题'} } },
};
