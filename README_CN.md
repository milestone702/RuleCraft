...
- **20+ 个内置输入插件** — 采集 Wi-Fi、电源、网络、进程、窗口、空闲时间、系统资源、磁盘、时间、会话、剪贴板、显示器、音量、CPU、区域等系统状态，以及 HTTP 请求、网络检测、手动触发、HTTP/TCP/UDP/WebSocket 入站、文件监控
- **20+ 个内置输出插件** — 桌面通知、电源方案切换、阻止休眠、执行程序、音量控制、亮度调节、锁定工作站、电源操作、壁纸设置、杀进程、Webhook/HTTP GET/HTTP POST、延时等待、文件写入、网卡控制、打开文件、窗口控制、截图、剪贴板设置
- **插件按类分组** — 系统类/硬件类/网络类/入站类/文件类/进程类/触发类，插件列表和编辑器中可按类筛选和搜索
- **AST 递归逻辑树** — 支持 AND/OR/NOT 嵌套条件判断，22 种运算符
- **触发阈值容错** — 支持条件连续成立 N 次后才触发输出（前 N-1 次仅写日志）
